(function(){
	...
	function makePattern(domain) {
		const domainStr = domain;
		return (cid, path) => { return "https://" + domainStr + "/ipfs/" + cid + "/" + path; };
	}

	const IPFS_GATEWAYS = [
		makePattern("gateway.ipfs.io"),
		makePattern("4everland.io"),
		makePattern("dweb.link"),
		makePattern("gw3.io"),
		makePattern("cloudflare-ipfs.com"),
		makePattern("cf-ipfs.com"),
		makePattern("w3s.link"),
		makePattern("ipfs.eth.aragon.network"),
		makePattern("storry.tv"),
		makePattern("nftstorage.link")
	];
